...
def hotflip_attack_multitask(x, gradientslist, mask, model, verbose=True):
    embedding = model.embedding
    x_embedding = embedding(x)
    embedding_matrix = embedding.weights[0]
    neg_dir_dot_grad_list, grads_norm_list = [], []
    for gradients in gradientslist:
        x_grad = gradients.values
        ...
        grads_norm = tf.einsum("ij,ij->i", grads, grads)
        grads_norm = np.einsum("j,j->j",mask,grads_norm)
        # **7 for multitask mode.
        neg_dir_dot_grad_list.append(neg_dir_dot_grad)
        grads_norm_list.append(grads_norm)
    grads_norm = np.mean(grads_norm_list, axis=0)
    which_token = np.argmax(grads_norm)
    neg_dir_dot_grad = tf.reduce_mean(neg_dir_dot_grad_list, keepdims=False, axis=0)
    best = tf.argmax(neg_dir_dot_grad, 2)
    if verbose:
        print("perturbed index:", which_token)
        print("perturbed token index:", best.numpy()[0][which_token])
...
def get_pred(m,x,y,verbose=True):
    ...
    if conf[0] < 0.5:
        ...
        pred = 1
    if verbose:
        print("real:", y, "pred:", pred, "conf", conf[0])
...
def attack(model,x,y,decode_review,loss_function,max_perturbed=5,verbose=True):
    ...
    with tf.GradientTape() as tape:
        output, same = get_pred(model,x,y,verbose=verbose)
    count = 0
    org = x
